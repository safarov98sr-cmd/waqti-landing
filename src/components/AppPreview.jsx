...
export default function AppPreview() {
  ...
  return (
    <section id="app-preview" className="relative py-20 sm:py-28 lg:py-32 overflow-hidden" style={{ background: '#070F1A' }}>
      {/* Side glows */}
      <div
        className="absolute left-0 top-1/2 -translate-y-1/2 w-[300px] h-[400px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at 0% 50%, rgba(239,159,39,0.04) 0%, transparent 70%)' }}
        aria-hidden="true"
      />
      <div
        className="absolute right-0 top-1/2 -translate-y-1/2 w-[300px] h-[400px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at 100% 50%, rgba(239,159,39,0.04) 0%, transparent 70%)' }}
        aria-hidden="true"
      />
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className={`flex justify-center mb-8 sm:mb-10 fade-in-section delay-1 ${inView ? 'visible' : ''}`}>
          <div
            className="flex gap-1 sm:gap-1.5 p-1 sm:p-1.5 rounded-2xl overflow-x-auto"
          ...
          >
            {screens.map((s, i) => (
              <button
                ...
                onMouseEnter={e => {
                  if (active !== i) e.currentTarget.style.color = 'rgba(255,255,255,0.75)'
                }}
                onMouseLeave={e => {
                  if (active !== i) e.currentTarget.style.color = 'rgba(255,255,255,0.45)'
                }}
              >
              ...
              return (
                ...
                >
                  ...
                  <span
                    ...
                    style={{
                      background: isActive ? 'linear-gradient(135deg,#EF9F27,#D4AF37)' : 'rgba(255,255,255,0.08)',
                      color: isActive ? '#07111A' : 'rgba(255,255,255,0.4)',
                    }}
                  >
                    {s.label}
                  </span>
              ...
              {screens.map((s, i) => (
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d={tabIcons[i]} stroke={active === i ? '#EF9F27' : 'rgba(255,255,255,0.4)'} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                  <span className="text-[10px] sm:text-xs font-semibold" style={{ color: active === i ? '#EF9F27' : 'rgba(255,255,255,0.35)' }}>
                    {s.label}
                  </span>
                </button>
              ))}
        ...
        <div className={`text-center mt-8 sm:mt-10 fade-in-section delay-3 ${inView ? 'visible' : ''}`}>
          <p className="text-sm sm:text-base max-w-md mx-auto px-4" style={{ color: 'rgba(255,255,255,0.45)' }}>
            {screens[active].desc}
          </p>
        </div>
...
function PhoneFrame({ children, active, size = 'md' }) {
  ...
  return (
    <div
      ...
      style={{
        border: active ? '2px solid rgba(239,159,39,0.4)' : '2px solid rgba(255,255,255,0.07)',
        boxShadow: active
          ? '0 32px 80px rgba(0,0,0,0.55), 0 0 50px rgba(239,159,39,0.12)'
          : '0 16px 40px rgba(0,0,0,0.4)',
        background: '#070F1A',
      }}
    >
      {/* Notch */}
      <div className="flex justify-center pt-2 pb-1" style={{ background: '#040A12' }}>
        <div className="w-14 h-3.5 rounded-full" style={{ background: '#02060C' }} />
      </div>
      {children}
    </div>
  )
...
function ScreenHome() {
  ...
  return (
    <div style={{ background: '#070F1A' }}>
      {/* App header */}
      <div className="px-4 pt-2 pb-4" style={{ background: '#040A12' }}>
      ...
      {/* Prayer list */}
      <div className="px-3 py-3 space-y-1.5" style={{ background: '#0A1628' }}>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-semibold" style={{
                color: p.active ? '#EF9F27' : p.done ? 'rgba(255,255,255,0.3)' : 'rgba(255,255,255,0.8)'
              }}>{p.name}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-medium" style={{
                color: p.active ? '#EF9F27' : p.done ? 'rgba(255,255,255,0.25)' : 'rgba(255,255,255,0.45)'
              }}>{p.time}</span>
              {p.done && (
              ...
              {p.active && <div className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: '#EF9F27' }} />}
            </div>
          </div>
        ))}
      </div>
      ...
      {/* Bottom nav */}
      <div className="flex justify-around py-2.5 px-3" style={{ background: '#040A12', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        {navPaths.map((d, i) => (
          <div key={i} style={{ color: i === 0 ? '#EF9F27' : 'rgba(255,255,255,0.28)' }}>
...
function ScreenPlanner() {
  ...
  return (
    <div style={{ background: '#0A1628', minHeight: 400 }}>
      {/* Header */}
      <div className="px-4 pt-3 pb-2 flex items-center justify-between"
        style={{ background: '#040A12', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        <div>
        ...
        <div className="w-6 h-6 rounded-full flex items-center justify-center" style={{ background: '#EF9F27' }}>
          <span className="text-[10px] font-bold" style={{ color: '#07111A' }}>+</span>
        </div>
      </div>
          ...
          <div key={bi}>
            ...
            <div className="flex items-center gap-2 mb-2">
              <div
                ...
                style={block.active
                  ? { background: '#EF9F27', boxShadow: '0 0 0 3px rgba(239,159,39,0.2)' }
                  : block.done
              ...
              >
                {(block.done || block.active) ? (
                  <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
                    <path d="M2 5l2.5 2.5 4-4" stroke={block.active ? '#07111A' : 'white'} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                ) : (
              ...
              <div className="flex items-center gap-1.5">
                <span className="text-xs font-bold" style={{
                  color: block.active ? '#EF9F27' : block.done ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.8)'
                }}>{block.prayer}</span>
                ...
                {block.active && (
                  <span className="text-[9px] rounded px-1 py-px font-bold"
                    style={{ background: '#EF9F27', color: '#07111A' }}>Сейчас</span>
                )}
              </div>
            </div>
            ...
            <div className="ml-3 pl-4 pb-3 space-y-1.5"
              style={{ borderLeft: '2px dashed rgba(255,255,255,0.08)' }}>
              <div className="text-[9px] font-medium mb-1.5" style={{ color: 'rgba(255,255,255,0.3)' }}>
                {block.blockLabel}
              </div>
              {block.tasks.map((t, ti) => (
                <div key={ti}
                  ...
                  style={t.done
                    ? { background: 'rgba(255,255,255,0.03)', opacity: 0.5 }
                    : { background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }
                  }
                >
                  <div
                    className="w-3.5 h-3.5 rounded flex-shrink-0 flex items-center justify-center"
                    style={t.done ? { background: '#EF9F27' } : { border: '1.5px solid rgba(255,255,255,0.2)' }}
                  >
                    {t.done && (
                      <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
                        <path d="M1.5 4l2 2 3-3" stroke="#07111A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                      </svg>
                    )}
                  </div>
...
function ScreenAnalytics() {
  ...
  return (
    <div style={{ background: '#0A1628', minHeight: 400 }}>
      {/* Header */}
      <div className="px-4 pt-3 pb-2.5" style={{ background: '#040A12', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <div className="px-3 py-3 space-y-3">
        {/* Streak */}
        <div className="rounded-2xl p-3" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
          <div className="flex gap-1">
            {Array.from({ length: 15 }).map((_, i) => (
              <div key={i} className="flex-1 h-1.5 rounded-full"
                style={{ background: i < 12 ? '#EF9F27' : 'rgba(255,255,255,0.1)' }} />
            ))}
          </div>
        </div>
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-md" style={{
                    height: `${h}%`,
                    background: isToday ? '#EF9F27' : d.count === 5 ? 'rgba(239,159,39,0.55)' : 'rgba(255,255,255,0.12)',
                  }} />
                  <span className="text-[8px] font-semibold" style={{ color: isToday ? '#EF9F27' : 'rgba(255,255,255,0.35)' }}>
                    {d.label}
                  </span>
                </div>
              )
        ...
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: 'Намазов за неделю', value: '29/35', color: '#EF9F27' },
            { label: 'Продуктивность',    value: '87%',   color: '#EF9F27' },
            { label: 'Задач выполнено',   value: '24',    color: '#60A5FA' },
            { label: 'Лучший день',       value: 'Пятница', color: '#C084FC' },
          ].map((s, i) => (
            <div key={i} className="rounded-xl p-2.5" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.07)' }}>
...
function ScreenCoach() {
  ...
  return (
    <div className="flex flex-col" style={{ background: '#0A1628', minHeight: 400 }}>
      {/* Header */}
      <div className="px-4 pt-3 pb-2.5 flex items-center gap-2.5"
        ...
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-label="ИИ коуч">
            ...
            <circle cx="8" cy="13" r="1.5" fill="#EF9F27" opacity="0.6" />
            <circle cx="12" cy="13" r="1.5" fill="#EF9F27" />
            <circle cx="16" cy="13" r="1.5" fill="#EF9F27" opacity="0.45" />
          </svg>
        </div>
        <div>
      ...
      <div className="flex-1 px-3 py-3 space-y-2 overflow-y-auto">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.from === 'user' ? 'justify-end' : 'justify-start'}`}>
            {m.from === 'ai' && (
              <div
                className="w-5 h-5 rounded-full flex items-center justify-center mr-1.5 mt-0.5 flex-shrink-0"
                style={{ background: 'rgba(239,159,39,0.15)', border: '1px solid rgba(239,159,39,0.3)', fontSize: 9 }}
              >
                ✦
              </div>
            )}
      ...
      {/* Input */}
      <div className="px-3 pb-3 pt-2" style={{ background: '#040A12', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="flex items-center gap-2 rounded-xl px-3 py-2"
          style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <span className="text-[10px] flex-1" style={{ color: 'rgba(255,255,255,0.3)' }}>
            Напиши сообщение...
          </span>
          <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: '#EF9F27' }}>
            <svg width="10" height="10" viewBox="0 0 10 10" fill="none">
              <path d="M2 8L8 5 2 2v2.5l4 .5-4 .5V8z" fill="#07111A" />
            </svg>
          </div>
        </div>
      </div>
