...
/* ── Islamic star helper (inline) ── */
const sp = (cx, cy) => starPolygon(cx, cy, 18, 7.5)

/* ── Decorative SVG Rosette ── */
function Rosette() {
    ...
    >
      ...
      {[R * 0.96, R * 0.88, R * 0.72, R * 0.56, R * 0.40].map((r, i) => (
        <circle key={i} cx={cx} cy={cy} r={r} fill="none"
          stroke="#10B981" strokeWidth="0.5" opacity={0.12 - i * 0.015} />
      ))}
      {/* Star rings */}
      {rings.map(({ r1, r2 }, i) => (
        <polygon key={i}
          points={starPolygon(cx, cy, r1, r2, 8)}
          fill="none"
          stroke="#10B981"
          strokeWidth="0.6"
          opacity={0.45 - i * 0.06}
      ...
      {octagons.map((r, i) => (
        <polygon key={i}
          points={regPolygon(cx, cy, r, 8, 22.5)}
          fill="none"
          stroke="#10B981"
          strokeWidth="0.5"
          opacity={0.28 - i * 0.04}
...
export default function Hero() {
  ...
  return (
    <section
      ...
      style={{ background: 'linear-gradient(140deg, #0A0F0D 0%, #0D1A12 55%, #0A0F0D 100%)' }}
    >
      {/* Canvas particles */}
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none" aria-hidden="true">
        <defs>
          <pattern id="hero-pattern" x="0" y="0" width="80" height="80" patternUnits="userSpaceOnUse">
            {/* Center star */}
            <polygon points={sp(40, 40)} fill="#10B981" opacity="0.13" />
            {/* Corner quarter-stars */}
            <polygon points={sp(0,  0)}  fill="#10B981" opacity="0.13" />
            <polygon points={sp(80, 0)}  fill="#10B981" opacity="0.13" />
            <polygon points={sp(0,  80)} fill="#10B981" opacity="0.13" />
            <polygon points={sp(80, 80)} fill="#10B981" opacity="0.13" />
            {/* Cardinal connectors */}
            <line x1="40" y1="0"  x2="40" y2="80" stroke="#10B981" strokeWidth="0.4" opacity="0.07" />
            <line x1="0"  y1="40" x2="80" y2="40" stroke="#10B981" strokeWidth="0.4" opacity="0.07" />
            {/* Diagonal connectors */}
            <line x1="0"  y1="0"  x2="80" y2="80" stroke="#F59E0B" strokeWidth="0.35" opacity="0.05" />
            <line x1="80" y1="0"  x2="0"  y2="80" stroke="#F59E0B" strokeWidth="0.35" opacity="0.05" />
          </pattern>
        </defs>
      ...
      <div className="absolute right-0 top-1/2 -translate-y-1/2 w-[500px] h-[500px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at 100% 50%, rgba(16,185,129,0.09) 0%, transparent 65%)' }}
        aria-hidden="true" />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 w-full">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
          ...
          <div ref={ref} className={`fade-in-section ${inView ? 'visible' : ''}`}>
            ...
            {/* Heading */}
            <h1 className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl leading-[1.08] mb-6 text-white">
              Управляй временем{' '}
              <span style={{
            ...
            <p className="text-lg sm:text-xl leading-relaxed mb-8 max-w-lg"
              ...
              Больше успевай — с нийей и осознанностью.
            </p>
            ...
            <div className="flex flex-wrap gap-3 sm:gap-4 mb-10">
              <a
                href="#pricing"
                className="inline-flex items-center gap-2.5 font-bold px-7 py-4 rounded-2xl cursor-pointer transition-all duration-200 btn-pulse"
                style={{
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
              {[
                ...
                <div key={i} className="flex items-center gap-1.5">
                  ...
                  {i < 2 && <span className="ml-2" style={{ color: 'rgba(255,255,255,0.15)' }}>·</span>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="animate-float relative z-10">
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: '0 32px 80px rgba(0,0,0,0.55), 0 0 50px rgba(16,185,129,0.12)',
                  background: '#0A0F0D',
                }}
              >
                {/* Notch */}
                ...
                <div className="px-3 py-3 space-y-1.5" style={{ background: '#0D1A12' }}>
                  <div className="text-[9px] font-bold uppercase tracking-wider mb-2"
                    style={{ color: 'rgba(255,255,255,0.25)' }}>Намазы сегодня</div>
                  {prayers.map(p => (
                    <div key={p.name}
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 animate-float"
        aria-hidden="true">
        <div className="w-px h-8 rounded-full" style={{ background: 'rgba(16,185,129,0.3)' }} />
