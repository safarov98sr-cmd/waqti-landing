...
function FAQItem({ faq, isOpen, onToggle }) {
  return (
    <div
      ...
      style={{
        border: `1.5px solid ${isOpen ? 'rgba(16,185,129,0.35)' : 'var(--card-border)'}`,
    ...
    >
      <button
        ...
        style={{ background: isOpen ? 'var(--faq-inner-bg)' : 'var(--faq-closed-bg)' }}
        aria-expanded={isOpen}
      >
        <span className="font-bold text-base"
          style={{ color: isOpen ? '#059669' : 'var(--text-h)' }}>
          {faq.question}
        </span>
        <span style={{ color: isOpen ? '#10B981' : 'var(--text-xmuted)', flexShrink: 0 }}>
      ...
      <div className="overflow-hidden"
        ...
        <div className="px-6 pb-6" style={{ background: 'var(--faq-inner-bg)' }}>
...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="relative py-24 sm:py-32 overflow-hidden" style={{ background: 'var(--bg-s2)' }}>
      <div className="absolute top-0 right-0 w-[400px] h-[250px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse at 100% 0%, rgba(16,185,129,0.04) 0%, transparent 65%)' }}
        aria-hidden="true" />
